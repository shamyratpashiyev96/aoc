    ...
    <div class="news_page__inner_wrapper">
        ...
        <div class="news_page__news_row flex_row">
            @foreach ($news_current_page as $news)
                <a href="{{ route('single_news_page',$news->id) }}" class="news_container flex_col">
        ...
        <div class="pagination_row flex_row">
            <a class="flex_row" style="align-items: center;" href="{{ route('news_page', ['page_num' => $prev_page,]) }}">
            ...
            @foreach ($pagination_array as $page)
                ...
                @else
                    <a href="{{ route('news_page', ['page_num' => $page]) }}" class="item @if($current_page == $page) active @endif">{{ $page }}</a>
                @endif
            @endforeach

            <a class="flex_row" style="align-items: center;" href="{{ route('news_page', ['page_num' => $next_page]) }}">
                <svg width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
